# Dashboard Pimpinan
Aplikasi Monitoring untuk Pimpinan
---
## Dashboard Pimpinan
- Dashboard Pimpinan merupakan aplikasi antar muka untuk monitoring dan mempermudah Pimpinan untuk melihat dan menilai kinerja OPD (Organisasi Perangkat Daerah). |
- Pada aplikasi ini Pimpinan dapat melihat kinerja OPD dalam bentuk prosentase, grafik dan list data. |

---

## Cara Penggunaan Aplikasi Dashboard Pimpinan
### 1 Cara Membuka Situs |
1. Bukalah aplikasi Dashboard Pimpinan melalui *web browser* dengan alamat url sebagai berikut : http://dashboard-01.dev.bantenprov.go.id/#/login . |
2. Kemudian tekan **Enter** pada tombol *keyboard* atau klik tombol **GO** pada browser. |
3. Maka pada layar akan tampak menu login situs Dashboard Pimpinan seperti ini: |

+++
![tampilan login](/assets/images/dashboard-login.png) 
Gambar 1. Menu Login Dashboard Pimpinan |

Masukan Email Id dan Password, setelah dimasuka dengan benar, klik tombol **Log in** atau tekan tombol **Enter** pada keyboard. Sehingga akan menampilkan halaman utama. |

---

### 2 Dashboard |
Pada halaman *Home* ini terdapat data dashboard dari OPD-OPD terkait yang ditampilkan dalam bentuk prosentase dan grafik. Prosentase dan grafik tersebut menampilkan data tiap bulan dari OPD terkait.|
[![Dashboard Halaman Depan](../images/dashboard-halaman-depan.png)](../images/dashboard-halaman-depan.png)
Gambar 2. Halaman Depan Dashboard Pimpinan |

+++
Pada Halaman ini jika *user* ingin melihat menu yang terdapat di Dashboard Pimpinan dapat mengklik tombol
![tombol lihat list menu](../images/tombol-list-menu.png) |

Sehingga *list menu* yang ada didalam apliksai dashboard pimpinan akan tampil seperti gambar dibawah ini:|

+++
[![Tampilan List Menu](../images/dashboard-list-menu.png)](../images/dashboard-list-menu.png)
Gambar 3. Tampilan List Menu Dashboard 
---




# beloman

##### 2.2.2.1 Dashboard Kepegawaian
Dashboard Kepegawaian dapat dilihat dengan mengklik submenu **Kepegawaian** pada menu Dashboard seperti ditunjukan dibawah ini:
[![klik kepegawaian](../images/dashboard-klik-kepegawaian.png)](../images/dashboard-klik-kepegawaian.png)

Maka akan muncul tampilan prosentase dan grafik dari submenu kepegawaian seperti gambar dibawah ini:
[![dashboard kepegawaian](../images/dashboard-kepegawaian.png)](../images/dashboard-kepegawaian.png)
Gambar 4. Dashboard Kepegawaian

###### 2.2.2.1.1 Dashboard Kepegawaian Berdasarkan Pegawai
Dashboard kepegawaian berdasarkan pegawai dapat dilihat dengan mengklik tab **Pegawai** seperti ditunjukan dibawah ini:
[![klik Pegawai](../images/kepegawaian-tab-pegawai.png)](../images/kepegawaian-tab-pegawai.png)

Maka aplikasi akan menampilkan prosentase dan grafik pegawai seperti ditunjukan pada gambar berikut:
[![Jumlah Pegawai](../images/kepegawaian-pegawai.png)](../images/kepegawaian-pegawai.png)
Gambar 5. Jumlah Pegawai

###### 2.2.2.1.2 Dashboard Kepegawaian Berdasarkan Kenaikan Pangkat Pegawai Otomatis
Dashboard kepegawaian berdasarkan Kenaikan Pangkat Pegawai Otomatis dapat dilihat dengan mengklik tab **Kenaikan Pangkat Pegawai Otomatis** seperti ditunjukan dibawah ini:
[![klik KPPO](../images/kepegawaian-tab-kppo.png)](../images/kepegawaian-tab-kppo.png)

Maka aplikasi akan menampilkan prosentase dan grafik dari Kenaikan Pangkat Pegawai Otomatis seperti ditunjukan pada gambar berikut:
[![Jumlah KKPO](../images/kepegawaian-kppo.png)](../images/kepegawaian-kppo.png)
Gambar 6. Jumlah Kenaikan pangkat Pegawai Otomatis

###### 2.2.2.1.3 Dashboard Kepegawaian Berdasarkan Pegawai yang Akan Pensiun
Dashboard kepegawaian berdasarkan Pegawai yang Akan Pensiun dapat dilihat dengan mengklik tab **Pegawai yang Akan Pensiun** seperti ditunjukan dibawah ini:
[![klik Pegawai akan Pensiun](../images/kepegawaian-tab-akan-pensiun.png)](../images/kepegawaian-tab-akan-pensiun.png)

Maka aplikasi akan menampilkan prosentase dan grafik dari Pegawai yang Akan Pensiun seperti ditunjukan pada gambar berikut:
[![Jumlah Pegawai yang akan Pensiun](../images/kepegawaian-akan-pensiun.png)](../images/kepegawaian-akan-pensiun.png)
Gambar 7. Jumlah Pegawai yang akan Pensiun

###### 2.2.2.1.4 Dashboard Kepegawaian Berdasarkan Pegawai Esselon
Dashboard kepegawaian berdasarkan Pegawai Esselon dapat dilihat dengan mengklik tab **Pegawai Esselon** seperti ditunjukan dibawah ini:
[![klik Pegawai Esselon](../images/kepegawaian-tab-esselon.png)](../images/kepegawaian-tab-esselon.png)

Maka aplikasi akan menampilkan prosentase dan grafik dari Pegawai Esselon seperti ditunjukan pada gambar berikut:
[![Jumlah Pegawai Esselon](../images/kepegawaian-esselon.png)](../images/kepegawaian-esselon.png)
Gambar 8. Jumlah Pegawai Esselon

###### 2.2.2.1.5 Dashboard Kepegawaian Berdasarkan Pangkat Pegawai
Dashboard kepegawaian berdasarkan Pangkat Pegawai dapat dilihat dengan mengklik tab **Pangkat Pegawai** seperti ditunjukan dibawah ini:
[![klik Pangkat Pegawai](../images/kepegawaian-pangkat-pegawai.png)](../images/kepegawaian-pangkat-pegawai.png)

Maka aplikasi akan menampilkan prosentase dan grafik dari Pangkat Pegawai seperti ditunjukan pada gambar berikut:
[![Jumlah Pegawai Esselon](../images/kepegawaian-pangkat-pegawai.png)](../images/kepegawaian-pangkat-pegawai.png)
Gambar 9. Jumlah Pangkat Pegawai

###### 2.2.2.1.6 Dashboard Kepegawaian Berdasarkan Usian ASN (Aparatur Sipil Negara)
Dashboard kepegawaian berdasarkan Usia ASN dapat dilihat dengan mengklik tab **Usia ASN** seperti ditunjukan dibawah ini:
[![klik Usia ASN](../images/kepegawaian-tab-usia-asn.png)](../images/kepegawaian-tab-usia-asn.png)

Maka aplikasi akan menampilkan prosentase dan grafik dari Usia Aparatur Sipil Negara (ASN) seperti ditunjukan pada gambar berikut:
[![Usia ASN](../images/kepegawaian-usia-asn.png)](../images/kepegawaian-usia-asn.png)
Gambar 10. Tampilan Usia ASN

##### 2.2.2.2 Dashboard Kependudukan
Dashboard Kependudukan dapat dilihat dengan mengklik submenu **Kependudukan** pada menu Dashboard seperti ditunjukan dibawah ini:
[![klik Kependudukan](../images/dashboard-klik-kependudukan.png)](../images/dashboard-klik-kependudukan.png)

Maka akan muncul tampilan prosentase dan grafik dari submenu kependudukan seperti gambar dibawah ini:
[![dashboard kependudukan](../images/dashboard-kependudukan.png)](../images/dashboard-kependudukan.png)
Gambar 11. Dashboard Kependudukan

###### 2.2.2.2.1 Dashboard Kependudukan Berdasarkan Penduduk
Dashboard kependudukan berdasarkan Penduduk dapat dilihat dengan mengklik tab **Penduduk** seperti ditunjukan dibawah ini:
[![klik Penduduk](../images/klik-tab-penduduk.png)](../images/klik-tab-penduduk.png)

Maka aplikasi akan menampilkan prosentase dan grafik penduduk seperti ditunjukan pada gambar berikut:
[![Jumlah Penduduk](../images/kependudukan-penduduk.png)](../images/kependudukan-penduduk.png)
Gambar 12. Jumlah Penduduk

##### 2.2.2.3 Dashboard Kesehatan
Dashboard Kesehatan dapat dilihat dengan mengklik submenu **Kesehatan** pada menu Dashboard seperti ditunjukan dibawah ini:
[![klik Kesehatan](../images/dashboard-klik-kesehatan.png)](../images/dashboard-klik-kesehatan.png)

Maka akan muncul tampilan prosentase dan grafik dari submenu kesehatan seperti gambar dibawah ini:
[![dashboard kesehatan](../images/dashboard-kesehatan.png)](../images/dashboard-kesehatan.png)
Gambar 13. Dashboard Kesehatan

###### 2.2.2.3.1 Dashboard Kesehatan Berdasarkan Tim Medis
Dashboard Kesehatan berdasarkan Tim Medis dapat dilihat dengan mengklik tab **Tim Medis** seperti ditunjukan dibawah ini:
[![klik Tim Medis](../images/klik-tab-tim-medis.png)](../images/klik-tab-tim-medis.png)

Maka aplikasi akan menampilkan prosentase dan grafik tim medis seperti ditunjukan pada gambar berikut:
[![Jumlah Tim Medis](../images/kesehatan-tim-medis.png)](../images/kesehatan-tim-medis.png)
Gambar 14. Jumlah Tim Medis

###### 2.2.2.3.2 Dashboard Kesehatan Berdasarkan Jumlah Fasilitas Kesehatan
Dashboard Kesehatan berdasarkan Jumlah Fasilitas Kesehatan dapat dilihat dengan mengklik tab **Jumlah Fasilitas Kesehatan** seperti ditunjukan dibawah ini:
[![klik Jumlah Fasilitas Kesehatan](../images/klik-tab-jumlah-fasilitas-kesehatan.png)](../images/klik-tab-jumlah-fasilitas-kesehatan.png)

Maka aplikasi akan menampilkan prosentase dan grafik fasilitas kesehatan seperti ditunjukan pada gambar berikut:
[![Jumlah Fasilitas Kesehatan](../images/kesehatan-jumlah-fasilitas-kesehatan.png)](../images/kesehatan-jumlah-fasilitas-kesehatan.png)
Gambar 15. Jumlah Fasilitas Kesehatan

###### 2.2.2.3.3 Dashboard Kesehatan Berdasarkan Jumlah Tempat Tidur
Dashboard Kesehatan berdasarkan Jumlah Tempat Tidur dapat dilihat dengan mengklik tab **Jumlah Tempat Tidur** seperti ditunjukan dibawah ini:
[![klik Jumlah Tempat Tidur](../images/klik-tab-jumlah-tempat-tidur.png)](../images/klik-tab-jumlah-tempat-tidur.png)

Maka aplikasi akan menampilkan prosentase dan grafik Jumlah Tempat Tidur seperti ditunjukan pada gambar berikut:
[![Jumlah Tempat Tidur](../images/kesehatan-jumlah-tempat-tidur.png)](../images/kesehatan-jumlah-tempat-tidur.png)
Gambar 16. Jumlah Tempat Tidur

##### 2.2.2.4 Dashboard Kependidikan
Dashboard Kependidikan dapat dilihat dengan mengklik submenu **Kependidikan** pada menu Dashboard seperti ditunjukan dibawah ini:
[![klik Kependidikan](../images/dashboard-klik-kependidikan.png)](../images/dashboard-klik-kependidikan.png)

Maka akan muncul tampilan prosentase dan grafik dari submenu Kependidikan seperti gambar dibawah ini:
[![dashboard Kependidikan](../images/dashboard-kependidikan.png)](../images/dashboard-kependidikan.png)
Gambar 17. Dashboard Kependidikan

###### 2.2.2.4.1 Dashboard Kependidikan Berdasarkan Pendidik
Dashboard Kependidikan Berdasarkan Pendidik dapat dilihat dengan mengklik tab **Pendidik** seperti ditunjukan dibawah ini:
[![klik Pendididik](../images/klik-tab-pendidik.png)](../images/klik-tab-pendididik.png)

Maka aplikasi akan menampilkan prosentase dan grafik Pendidik seperti ditunjukan pada gambar berikut:
[![Jumlah Pendididik](../images/kependidikan-pendidik.png)](../images/kependidikan-pendidik.png)
Gambar 18. Jumlah Pendididik

##### 2.2.2.5 Dashboard Epormas
Dashboard Epormas dapat dilihat dengan mengklik submenu **Epormas** pada menu Dashboard seperti ditunjukan dibawah ini:
[![klik Epormas](../images/dashboard-klik-epormas.png)](../images/dashboard-klik-epormas.png)

Maka akan muncul tampilan prosentase dan grafik dari submenu Epormas seperti gambar dibawah ini:
[![dashboard Epormas](../images/dashboard-epormas.png)](../images/dashboard-epormas.png)
Gambar 19. Dashboard Epormas

###### 2.2.2.5.1 Dashboard Epormas Berdasarkan Jumlah Laporan
Dashboard Epormas Berdasarkan Jumlah Laporan dapat dilihat dengan mengklik tab **Jumlah Laporan** seperti ditunjukan dibawah ini:
[![klik Jumlah Laporan](../images/klik-tab-jumlah-laporan.png)](../images/klik-tab-jumlah-laporan.png)

Maka aplikasi akan menampilkan prosentase dan grafik Jumlah Laporan seperti ditunjukan pada gambar berikut:
[![Jumlah Laporan](../images/epormas-jumlah-laporan.png)](../images/epormas-jumlah-laporan.png)
Gambar 20. Jumlah Laporan

#### 2.2.3 Data Kepegawaian
Pada menu Data Kepegawaian ini akan memperlihatkan data master dari sub menu Kepegawaian, yang antara lain:
1. List Pegawai
2. List Pegawai KPO
3. List Pegawai Esselon
4. List Pangkat Pegawai
5. List Usia ASN
6. List Pegawai yang akan Pensiun

##### 2.2.3.1 List Pegawai
List Pegawai dapat dilihat dengan cara mengklik sub menu "List Pegawai" seperti pada gambar dibawah ini:
[![Klik List Pegawai](../images/klik-list-pegawai.png)](../images/klik-list-pegawai.png)

Maka aplikasi akan menampilkan tabel list pegawai seperti gambar dibawah ini:
[![Tabel List Pegawai](../images/data-kepegawaian-list-pegawai.png)](../images/data-kepegawaian-list-pegawai.png)
Gambar 21. Tabel List Pegawai

Didalam tabel ini *user* dapat melihat data jumlah pegawai, pada halaman ini *user* juga dapat menambahkan, mengedit dan menghapus data jumlah pegawai dari aplikasi Dashboard Pimpinan ini.

###### 2.2.3.1.1 Tambah Data Jumlah Pegawai
*User* dapat menambahkan data di sub menu List Pegawai ini dengan cara mengklik tombol "Create" seperti gambar dibawah ini:

[![Tombol Klik Tambah Data](../images/tombol-tambah-data.png)](../images/tombol-tambah-data.png)

Sehingga akan menampilkan *Form Tambah Jumlah Pegawai* seperti gambar dibawah ini:

[![Form Tambah Pegawai](../images/form-kepegawaian-tambah-pegawai.png)](../images/form-kepegawaian-tambah-pegawai.png)
Gambar 22. Tambah Jumlah Pegawai

Pada halaman ini *user* dapat melihat *form* untuk menginput data jumlah pegawai, setelah mengisi *form, user* dapat mengklik tombol "Submit" untuk menyimpan data, tombol "Reset" untuk menghapus data yang sudah diinput atau tombol "Back" untuk kembali ke halaman List Pegawai tanpa menyimpan data.

###### 2.2.3.1.2 Edit Data Jumlah Pegawai
*User* dapat mengedit data dari data master Jumlah Pegawai dengan cara mengklik tombol "Edit" pada kolom action seperti gambar dibawah ini:

[![tombol edit](../images/tombol-edit-data.png)](../images/tombol-edit-data.png)

Sehingga aplikasi akan menampilkan *Form Edit Jumlah Pegawai* seperti gambar dibawah ini:

[![Tampilan Edit Jumlah Pegawai](../images/form-kepegawaian-edit-pegawai.png)](../images/form-kepegawaian-edit-pegawai.png)
Gambar 23. Edit Jumlah Pegawai

Pada halaman ini *user* dapat melihat *form edit Jumlah Pegawai*, setelah mengedit data jumlah pegawai, *user* dapat mengklik "Submit" untuk menyimpan hasil editan kedalam database atau mengklik "Reset" untuk membersihkan data editan atau dapat mengklik tombol "Back" untu kembali ke halaman List Pegawai tanpa menyimpan data.

###### 2.2.3.1.3 Delete Data Jumlah Pegawai
Didalam List Pegawai *user* dapat menghapus jumlah pegawai yang tidak diinginkan dengan mengklik tombol "Delete" pada kolom action dari tabel list pegawai seperti gambar dibawah ini:

[![tombol hapus data](../images/tombol-delete-data.png)](../images/tombol-delete-data.png)

###### 2.2.3.1.4 Export Data Jumlah Pegawai
Didalam List Pegawai *user* dapat mengexport data jumlah pegawai dengan mengklik icon "Download" pada sebelah kanan atas tabel list pegawai seperti gambar dibawah ini:

[![tombol hapus data](../images/tombol-export-data.png)](../images/tombol-export-data.png)
